...
describe("CallHierarchy view", () => {
  ...
  it("renders title", async () => {
    ...
    const content = view.querySelector("atom-ide-call-hierarchy-item");
    expect(
      content.children[0].querySelector(":scope>div span:nth-child(2)")
        .innerHTML,
    ).toEqual("name1");
    expect(
      content.children[1].querySelector(":scope>div span:nth-child(2)")
        .innerHTML,
    ).toEqual("name2");
    expect(
      content.children[2].querySelector(":scope>div span:nth-child(2)")
        .innerHTML,
    ).toEqual("name3");
  });
  it("renders child", async () => {
    ...
    for (const child of content.children) {
      expect(
        child.querySelector(":scope>atom-ide-call-hierarchy-item").children
          .length,
      ).toEqual(3);
    }
  });
  it("renders child content", async () => {
    ...
    for (const child of content.children) {
      const childContent = child.querySelector(
        ":scope>atom-ide-call-hierarchy-item",
      );
      expect(
        childContent.children[0].querySelector(":scope>div span:nth-child(2)")
          .innerHTML,
      ).toEqual("name1");
      expect(
        childContent.children[1].querySelector(":scope>div span:nth-child(2)")
          .innerHTML,
      ).toEqual("name2");
      expect(
        childContent.children[2].querySelector(":scope>div span:nth-child(2)")
          .innerHTML,
      ).toEqual("name3");
    }
  });
  it("renders child content", async () => {
    await view.showCallHierarchy(editor, point);
    const content = view.querySelector("atom-ide-call-hierarchy-item");
    const childContent = content.children[0].querySelector(
      ":scope>atom-ide-call-hierarchy-item",
    );
    expect(childContent.querySelector("atom-ide-call-hierarchy-item")).toEqual(
      null,
    );
    childContent.querySelector(":scope>div>div").dispatchEvent(
      new Event("click"),
    );
    waitsFor("item add", function () {
      return childContent.querySelector("atom-ide-call-hierarchy-item");
    }, 100);
    runs(() => {
      expect(
        childContent.querySelector("atom-ide-call-hierarchy-item").children
          .length,
      ).toEqual(3);
    });
  });
