...
export class CallHierarchyView extends HTMLElement {
  ...
  getIconName = () => "link";
  static getStatus(
    data: CallHierarchy<CallHierarchyType> | statusKey | null | undefined,
  ): statusKey | "valid" {
  ...
  #updateCallHierarchyView = async (
    newData: CallHierarchy<CallHierarchyType> | statusKey | null | undefined,
  ) => {
    ...
    if (currentStatus === "valid") {
      ...
      const item = new CallHierarchyViewItem(
        newData as CallHierarchy<CallHierarchyType>,
      );
...
class CallHierarchyViewItem<T extends CallHierarchyType> extends HTMLElement {
  ...
  static isEmpty(
    callHierarchy: CallHierarchy<CallHierarchyType> | undefined,
  ): callHierarchy is undefined {
  ...
  constructor(callHierarchy: CallHierarchy<T> | undefined) {
    ...
    if (CallHierarchyViewItem.isEmpty(this.#callHierarchy)) {
      this.innerHTML =
        `<div class="call-hierarchy-no-data">No result was found.</span>`;
      return;
    }
    this.append(...this.#callHierarchy.data.map((item, i) => {
...
customElements.define(
  "atom-ide-call-hierarchy-status-item",
  CallHierarchyViewStatusItem,
);
